...
# WAIT is the time before we trigger a
# repeat notification 
WAIT = 1 # seconds
...
class Backend():
    def __init__(self):
        ...
        users_group_map = {'ID': 'user group map', 'Data': dict()}
        broadcast_times = {'ID': 'broadcast times', 'Data': dict()}
        min_threshes = {'ID': 'min threshes', 'Data': dict()}
    ...
    def _update_sync(self, data_id, data):
        print self._get_sync(data_id)
        print data_id
        print data
        self.sync_data.update({'ID': data_id}, {'ID': data_id, 'Data': data})
        print data
        print self._get_sync(data_id)
        raise "blah"
    ...
    def _remove_user(self, group_id, phone_id):
        ...
        users_group_map = self._get_sync('user group map')
        users_group_map.pop(phone_id, None)
        self._update_sync('user group map', users_group_map)
    ...
    def update_location(self, group_id, phone_id, location):
        ...
        broadcast_times = self._get_sync('broadcast time')
        min_threshes = self._get_sync('min threshes')
        ...
        self._update_sync('broadcast time', broadcast_times)
        self._update_sync('min threshes', min_threshes)
